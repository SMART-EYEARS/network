...
![HttpMultipleRequest.png](./images/HttpMultipleRequest.png)    
   
사진을 보면 왼쪽이 `multiple request`라 헷갈릴 수 있는데 이름을 자세히보면 `Connection`이라 되어있다.             
즉, `Request`가 아닌 `Conncetion`이므로, 왼쪽이 **HTTP_1.0**, **오른쪽이 HTTP_1.1 이다.**            
   
`multiple request`란 한번에 연결에 여러 Request를 보낼 수 있다는 뜻이고      
`파이프라인 방식`이란, 파이프/스트림 처럼 데이터 흐름이 `클라이언트-서버-클라이언트`와 같은 형태를 의미한다.     
       
이러한 형태를 `Keep-alive`라 부르며,          
한 번 맺어졌던 연결을 끊지 않고 지속적으로 유지하여      
**불필요한 `hand-shake`를 줄여 성능을 개선할 수 있다.**           

하지만, 이러한 `HTTP_1.1`도 문제가 있는데   
바로 하나의 요청에 하나의 응답만을 보내는 것이다.   
     
응답 속도가 늦으면 그만큼 Client에서는 다음 작업을 진행하지 못하고      
만약 도중에 응답을 가로챈다면 Client는 무한정 대기상태가 된다.         
